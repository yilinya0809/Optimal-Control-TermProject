...
while itr <= iter_num
    if itr == 1
        ...
        [time_1, z] = ode45(@(t,z) BT_eqn(t,z,alpha,Gamma_max), [t0 t1], z0);
        x1_ode = z(end,1:6);
    end
    ...
    for i = [1,2,4,5]
        ...
        z0_del(:,i) = [x0; lamb0_del(:,i)];
        [~, z_del_rx] = ode45(@(t,z) BT_eqn(t,z, alpha, Gamma_max), [t0 t1], z0_del(:,1));
        [~, z_del_ry] = ode45(@(t,z) BT_eqn(t,z, alpha, Gamma_max), [t0 t1], z0_del(:,2));
        [~, z_del_vx] = ode45(@(t,z) BT_eqn(t,z, alpha, Gamma_max), [t0 t1], z0_del(:,4));
        [~, z_del_vy] = ode45(@(t,z) BT_eqn(t,z, alpha, Gamma_max), [t0 t1], z0_del(:,5));

        x1_del(1,:) = z_del_rx(end,1:6);
        x1_del(2,:) = z_del_ry(end,1:6);
        x1_del(4,:) = z_del_vx(end,1:6);
        x1_del(5,:) = z_del_vy(end,1:6);
    ...
    z0_new = [x0; lamb0_new];
    [~,z_new] = ode45(@(t,z) BT_eqn(t,z,alpha,Gamma_max), [t0 t1], z0_new);
...
hold on
Gamma_max = 5*1e-4*ones(length(time_1_unn));
plot(time_step1, Gamma_max);